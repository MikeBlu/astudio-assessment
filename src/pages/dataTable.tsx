...
function DataTable({dataLabel, tableFields, filterFields}: {dataLabel: string, tableFields: string[], filterFields?: string[][]}) {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      axios.get(`https://dummyjson.com/${dataLabel}/${query}`)
        .then((res) => {
          setData(res.data);
        })
    ...
    getData();
  }, [dataLabel, pageSize, currentPage, filter]);

  const initRows = (columns: string[]) => {
    var rows = [];
    if (dataLabel in data) {
      ...
      for (var record of data[dataLabel]) {
        if (searchTerm !== "*" && !matchesRow(record, searchTerm, columns)) continue;
        var cols = [];
        for (let field of columns) {
            cols.push(<td>{record[field] || "N/A"}</td>);
        }
         rows.push(
          <tr key={record['id']} className='[&>td]:border-2 [&>td]:border-grey [&>td]:py-2 [&>td]:px-3 content-center hover:bg-grey'>
            {cols}
          </tr>
        )
      }
  ...
  const matchesRow = (row: any, term: string, keys: string[]) => {
      ...
      <thead className='content-center bg-blue'>
        <tr className='text-center [&>th]:border-2 [&>th]:border-grey [&>th]:py-2'>
            ...
            for (let field of tableFields) {
                tableHeaders.push(<th>{field.toUpperCase()}</th>);
            }
            return tableHeaders;
            })()}
        </tr>
